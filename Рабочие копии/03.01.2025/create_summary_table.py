...
def daily_metrics():
    ...
    try:
        ...
        metrics = {
            'labels': ['Хэшрейт LTC', 'Хэшрейт BTC', 'Асики LTC', 'Асики BTC', 'Всего устройств', 'Новые устройства', 'Активные устройства', 'Неактивные устройства'],
            'today': df[df['date'] == today].sum(numeric_only=True).fillna(0).tolist(),
            'yesterday': df[df['date'] == yesterday].sum(numeric_only=True).fillna(0).tolist(),
            'growth': []
        }
