...
def manage_keys(user_id):
    ...
    conn = sqlite3.connect(SECONDARY_DATABASE_PATH)
    cursor = conn.cursor()
    available_keys = cursor.execute("SELECT key, status, start_date, end_date, tariff_id FROM user_keys").fetchall()
    conn.close()
    ...
    <head>
        ...
        <style>
            ...
            .sort-arrow { font-size: 12px; margin-left: 5px; color: #777; }
            .modal { display: none; position: fixed; z-index: 100; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4); }
            .modal-content { background-color: #fefefe; margin: 2% auto; padding: 20px; border: 1px solid #888; width: 90%; max-width: 1200px; height: 80%; max-height: 90vh; overflow-y: auto; }
    ...
    <body>
        <div class="container">
            ...
            <a href="{{ url_for('edit_user', user_id=user.id) }}" class="btn btn-secondary mb-3">Назад к редактированию пользователя</a>
            ...
            <div class="table-container">
                <table id="keyTable" class="table">
                    <thead>
                        <tr>
                            <th class="sortable" onclick="sortTable(0)">Ключ <span id="arrow-key" class="sort-arrow">▲▼</span></th>
                            <th class="sortable" onclick="sortTable(1)">Статус <span id="arrow-status" class="sort-arrow">▲▼</span></th>
                            <th class="sortable" onclick="sortTable(2)">Дата начала <span id="arrow-start" class="sort-arrow">▲▼</span></th>
                            <th class="sortable" onclick="sortTable(3)">Дата окончания <span id="arrow-end" class="sort-arrow">▲▼</span></th>
                            <th class="sortable" onclick="sortTable(4, true)">Осталось дней <span id="arrow-days" class="sort-arrow">▲▼</span></th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <script>
                ...
                function populateOptions(selectElement, filter = "") {
                    const filteredKeys = availableKeys.filter(key => 
                        key[0].toLowerCase().includes(filter.toLowerCase())
                    );
                    ...
                    filteredKeys.forEach(key => {
                        ...
                        option.value = key[0];
                        option.text = `${key[0]} (Статус: ${key[1]}, Начало: ${key[2]}, Конец: ${key[3]})`;
                        selectElement.appendChild(option);
                    });
                }

                const sortOrder = Array(5).fill("asc");
                ...
                function sortTable(columnIndex, numeric = false) {
                    ...
                    rows.sort((rowA, rowB) => {
                        ...
                        const valueA = numeric ? parseInt(cellA) || 0 : cellA.toLowerCase();
                        const valueB = numeric ? parseInt(cellB) || 0 : cellB.toLowerCase();
                        
                        if (valueA < valueB) return sortOrder[columnIndex] === "asc" ? -1 : 1;
                        if (valueA > valueB) return sortOrder[columnIndex] === "asc" ? 1 : -1;
                        return 0;
                    });
                ...
                function updateSortArrows(columnIndex) {
                    ...
                    arrowElements.forEach((arrow, index) => {
                        if (index === columnIndex) {
                            arrow.textContent = sortOrder[columnIndex] === "asc" ? "▲" : "▼";
                        } else {
                            arrow.textContent = "▲▼";
                        }
                    });
                ...
                document.addEventListener("DOMContentLoaded", () => {
                    const headers = document.querySelectorAll("#keyTable th.sortable");
                    headers.forEach((header, index) => {
                        const numeric = index === 4; 
                        header.addEventListener("click", () => sortTable(index, numeric));
                    });
                });
                ...
                $(document).ready(function() {
                    $("#openAddKeyModal").click(function() {
                        ...
                        populateOptions(document.getElementById("addKeySelect")); 
                    });
                    ...
                    $("#openDetachKeyModal").click(function() {
                        ...
                        populateOptions(document.getElementById("detachKeySelect")); 
                    });
                    ...
                    $("#addKeySearch").on("input", function() {
                        ...
                        populateOptions(document.getElementById("addKeySelect"), filter);
                    });

                    $("#detachKeySearch").on("input", function() {
                        const filter = $(this).val();
                        populateOptions(document.getElementById("detachKeySelect"), filter);
                    });
                });
            </script>
        </div>
