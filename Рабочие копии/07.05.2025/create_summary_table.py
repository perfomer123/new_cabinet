...
def keys_list():
    """
    Страница «Управление ключами»:
    - Отображает список всех ключей (из user_keys).
    - Поиск по параметру ?search=... (фильтр по номеру ключа).
    - Показ, привязан ли ключ к пользователю (email/phone_number) или нет.
    - Ссылка на детальную страницу ключа.
    """
    role = g.user.role
    users, user_tariffs, tariffs, user_keys, miner_data = get_data_for_user(role)
    ...
    # Фильтрация по подстроке в номере ключа (колонка 'key')
    if search_term:
        filtered_keys = user_keys[user_keys['key'].str.contains(search_term, case=False, na=False)]
    else:
        filtered_keys = user_keys

    # Объединяем, чтобы узнать, к какому пользователю ключ привязан (email, phone_number)
    merged_df = filtered_keys.merge(
        users, left_on='user_id', right_on='id', how='left', suffixes=('', '_user')
    )

    # Готовим данные к шаблону
    keys_info = []
    for idx, row in merged_df.iterrows():
        # --------------------------------------
        # Безопасно обрабатываем start_date/end_date,
        # т.к. они могут быть строками
        # --------------------------------------
        raw_start_date = row['start_date']
        raw_end_date = row['end_date']

        # Обработка start_date
        if pd.notnull(raw_start_date):
            if isinstance(raw_start_date, (pd.Timestamp, datetime)):
                start_date_str = raw_start_date.strftime('%Y-%m-%d')
            else:
                # Пытаемся преобразовать строку в дату
                converted = pd.to_datetime(raw_start_date, errors='coerce')
                start_date_str = converted.strftime('%Y-%m-%d') if pd.notnull(converted) else ''
        else:
            start_date_str = ''

        # Обработка end_date
        if pd.notnull(raw_end_date):
            if isinstance(raw_end_date, (pd.Timestamp, datetime)):
                end_date_str = raw_end_date.strftime('%Y-%m-%d')
            else:
                # Пытаемся преобразовать строку в дату
                converted = pd.to_datetime(raw_end_date, errors='coerce')
                end_date_str = converted.strftime('%Y-%m-%d') if pd.notnull(converted) else ''
        else:
            end_date_str = ''

        # Если user_id нет, значит ключ не привязан
        user_id = row['user_id'] if pd.notnull(row['user_id']) else None
        if user_id:
            user_email = row['email']
            phone_number = row['phone_number']
        else:
            user_email = 'Не привязан'
            phone_number = ''
    ...
    # HTML-шаблон (inline, без сокращений)
    template = '''
    <!DOCTYPE html>
    <html lang="ru">
    <head>
    ...
    <div class="container my-4">
        <h1 class="text-center">Управление ключами</h1>

        <!-- Форма поиска (по номеру ключа) -->
        <form method="GET" action="/keys" class="form-inline mb-3">
            <div class="form-group mr-2">
                <label for="search" class="mr-2">Поиск по ключу:</label>
                <input type="text" name="search" id="search" class="form-control" placeholder="Введите ключ"
                       value="{{ search_term }}">
            </div>
            ...
            <table class="table table-bordered">
                ...
                {% for k in keys_info %}
                    <tr>
                        ...
                        <td>
                            {% if k.user_id %}
                                {{ k.user_email }} ({{ k.phone_number }})
                            {% else %}
                                Не привязан
                            {% endif %}
                        </td>
                        <td>
                            <!-- Ссылка на «карточку» ключа -->
                            <a href="{{ url_for('key_details', key_uid=k.key) }}" class="btn btn-info btn-sm">Открыть</a>
                        </td>
                    </tr>
                {% endfor %}
                </tbody>
    ...
    '''
    return render_template_string(template, keys_info=keys_info, search_term=search_term)


@app.route('/key/<key_uid>', methods=['GET','POST'])
@auth_required
@role_required('admin')
def key_details(key_uid):
    """
    Эндпоинт /key/<key_uid>.
      - При GET: отображает инфу о ключе, даже если user_id=NULL (ключ отвязан).
      - При POST: сбрасывает ключ (user_id=NULL, status='new', start_date=NULL, end_date=NULL),
        вызывая внутреннюю def reset_key_subscription.
    """

    def reset_key_subscription(key_str):
        """
        Локальная функция (аналог extend_key_subscription),
        но обнуляет user_id, start_date, end_date, ставит status='new'.
        Возвращает (result_string, status_code).
        """

        try:
            # Укажите тот же путь, где реально лежит таблица user_keys:
            db_path = "/root/miner-data/file.db"  # или ваш нужный путь
            ...
            with engine.begin() as conn:
                query = text("""
                    ...
                    WHERE key = :k
                """)
                conn.execute(query, {"k": key_str})

            return ("Key reset successful", 200)

        except Exception as e:
            return (f"Error resetting key: {e}", 500)

    # ----------------------------------------------------------------
    # 1. Если POST => сброс ключа
    # ----------------------------------------------------------------
    if request.method == 'POST':
        reset_key_str = request.form.get('reset_key')
        if reset_key_str:
            # Берём DataFrame'ы (аналогично вашей логике «продления»)
            role = g.user.role
            users, user_tariffs, tariffs, user_keys, miner_data = get_data_for_user(role)

            # Проверяем, что такой ключ есть в DataFrame user_keys
            key_row = user_keys[user_keys['key'] == reset_key_str]
            if key_row.empty:
                return f"Ключ '{reset_key_str}' не найден в user_keys DataFrame", 404

            user_id_current = key_row.iloc[0].get('user_id', None)
            app.logger.info(f"[key_details] Сброс ключа '{reset_key_str}', user_id={user_id_current}")

            result, status_code = reset_key_subscription(reset_key_str)
            app.logger.info(f"[key_details] reset_key_subscription -> {result}, status_code={status_code}")
    ...
    # ---- Соединяем user_keys с тарифами (если нужно) ----
    user_keys_with_tariffs = user_keys.merge(
        tariffs, left_on='tariff_id', right_on='tariff_id',
        how='left', suffixes=('', '_tariffs')
    )
    # ---- Соединяем (LEFT) user_keys_with_tariffs с users ----
    #    чтобы ключ без user_id все равно оставался
    merged_data = user_keys_with_tariffs.merge(
        users, left_on='user_id', right_on='id',
        how='left',  # LEFТ merge: сохраним ключ даже если user_id=NULL
        suffixes=('', '_users')
    )

    # Фильтруем по ключу
    key_data = merged_data[merged_data['key'] == key_uid]
    if key_data.empty:
        # Если вообще нет строки с таким key, значит ключ не существует
        return f"Ключ '{key_uid}' не найден (merged_data)", 404

    row = key_data.iloc[0]

    # Достаём поля
    user_id_val = row['user_id'] if pd.notnull(row['user_id']) else None
    status_val = row['status'] if 'status' in row else 'unknown'
    start_date_val = row['start_date'] if 'start_date' in row else None
    end_date_val = row['end_date'] if 'end_date' in row else None
    ...
    start_date_str = fmt_date(start_date_val)
    end_date_str = fmt_date(end_date_val)
    ...
    <head>
      <meta charset="UTF-8">
      <title>Ключ {{ key_uid }}</title>
      <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
    </head>
    <body class="p-3">
      <h2>Ключ: {{ key_uid }}</h2>
      <p><b>user_id:</b> {{ user_id_val if user_id_val else 'Не привязан' }}</p>
      <p><b>status:</b> {{ status_val }}</p>
      <p><b>start_date:</b> {{ start_date_str }}</p>
      <p><b>end_date:</b> {{ end_date_str }}</p>
      <hr>

      <!-- Кнопка для сброса -->
      <form method="POST">
        <input type="hidden" name="reset_key" value="{{ key_uid }}" />
        <button type="submit" class="btn btn-danger">Сбросить ключ</button>
      </form>

      <br>
      <a href="/keys" class="btn btn-secondary">Вернуться к списку</a>
    </body>
    </html>
    '''
