...
from flask import Flask, render_template_string, jsonify, request, redirect, url_for, flash, session, make_response
...
SECRET_KEY = 'hd82894rhfd2347e8*FG^&*grtffg'
...
def auth_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            if user:
                login_user(user)  # Устанавливаем текущего пользователя
                return f(*args, **kwargs)
...
# Декоратор для проверки роли пользователя с перенаправлением на логин при ошибках
def role_required(*roles):
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            # Проверка, авторизован ли пользователь и соответствует ли его роль
...
# Маршрут для выхода с переходом на страницу логина
@app.route('/logout')
@auth_required
def logout():
    logout_user()
...
@app.route('/dashboard')
@auth_required
@role_required('admin', 'partner', 'manager', 'supervisor')  # Разрешаем доступ разным ролям
def dashboard():
    dashboard_html = '''
...
def register():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            flash('Этот email уже используется. Пожалуйста, выберите другой.')
            return redirect(url_for('register'))

        # Проверка на существующее имя пользователя
        if User.query.filter_by(username=username).first():
            flash('Имя пользователя уже занято. Пожалуйста, выберите другое.')
            return redirect(url_for('register'))

        # Создаем нового пользователя с ролью 'partner'
        new_user = User(
            username=username,
            email=email,
...
def edit_user(user_id):
    ...
    tariffs = Tariff.query.all()  # Получаем все тарифы для отображения в форме
    ...
    if request.method == 'POST':
        ...
        if action == 'update_user':
            ...
            user.tariff_id = request.form['tariff_id']  # Обновляем tariff_id пользователя

            selected_tariff = Tariff.query.get(user.tariff_id)  # Получаем выбранный тариф
            ...
            # Ваш существующий код для добавления ключей
            pass
        
        elif action == 'detach_keys':
            # Ваш существующий код для отвязки ключей
            pass
        
        elif action == 'add_manual_key':
            # Ваш существующий код для добавления ключа вручную
            pass
    ...
    <body>
        <div class="form-container">
            ...
            <!-- Остальная часть страницы: управление ключами, модальные окна и т.д. -->
        </div>

        <!-- Модальные окна и скрипты -->
        <!-- Здесь вы можете добавить код для модальных окон, если они есть в вашем исходном коде -->
    </body>
    </html>
    ''', user=user, tariffs=tariffs, supervisors=supervisors, managers=managers, current_supervisor_id=current_supervisor_id, current_manager_id=current_manager_id, processed_user_keys=processed_user_keys, available_keys=available_keys, available_tariffs_dict=available_tariffs_dict)
...
def notify_user_about_key_status():
    ...
    try:
        ...
        cursor.execute("""
            SELECT k.id, k.user_id, k.key, k.end_date, u.email, u.phone_number, k.last_notification, k.device_id
            FROM user_keys k
        ...
        for key in keys:
            ...
            device_id = key[7]  # Уникальный идентификатор устройства

            if not end_date_str:
                logger.warning(f"Подписка для устройства {device_id} не имеет даты окончания.")
                continue
            ...
            if last_notification_str:
                ...
                if last_notification == current_date:
                    logger.info(f"Подписка для устройства {device_id} проверена. Уведомление не требуется, до окончания осталось {remaining_days} дней.")
                    continue

            if remaining_days == 5:
                send_notification(key_value, email, phone_number, f"Подписка для устройства {device_id} истекает через 5 дней.", include_link=True)
                logger.info(f"Уведомление отправлено для устройства {device_id}. До окончания действия осталось 5 дней.")
            elif remaining_days == 2:
                send_notification(key_value, email, phone_number, f"Подписка для устройства {device_id} истекает через 2 дня.", include_link=False)
                logger.info(f"Уведомление отправлено для устройства {device_id}. До окончания действия осталось 2 дня.")
            elif remaining_days == 0:
                send_notification(key_value, email, phone_number, f"Подписка для устройства {device_id} истекла. Доступ приостановлен.", include_link=False)
                logger.info(f"Уведомление отправлено для устройства {device_id}. Подписка истекла сегодня.")
            else:
                logger.info(f"Подписка для устройства {device_id} проверена. Уведомление уже было отправлено сегодня.")
...
def send_notification(key_value, email, phone_number, message, include_link=False):
    ...
    if send_email(email, f"Уведомление о подписке устройства {device_id}", email_message):
...
if __name__ == '__main__':
    ...
    scheduler.add_job(notify_user_about_key_status, 'interval', minutes=720)
