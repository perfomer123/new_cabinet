...
def handle_status_change(user_key, old_status):
    try:
        print(f"handle_status_change called for key {user_key.key}, old_status={old_status}, new_status={user_key.status}")

        partner = User.query.filter_by(id=user_key.user_id).first()
        if not partner:
            print(f"No partner found for user_key {user_key.id}")
            return
        ...
        if manager_association:
            ...
            if not manager:
                print(f"No manager found for manager association {manager_association.manager_id}")
        ...
        if manager:
            ...
            if supervisor_association:
                ...
                if not supervisor:
                    print(f"No supervisor found for supervisor association {supervisor_association.supervisor_id}")

        # Получение тарифа партнёра
        partner_tariff = Tariff.query.filter_by(id=partner.tariff_id).first()
        if not partner_tariff:
            print(f"No partner tariff found for partner {partner.id}")
            return
        ...
        if manager:
            ...
            if not manager_tariff:
                print(f"No manager tariff found for manager {manager.id}")
        ...
        if supervisor:
            ...
            if not supervisor_tariff:
                print(f"No supervisor tariff found for supervisor {supervisor.id}")

        # Логика начисления при смене статуса
        if old_status == 'new' and user_key.status == 'active':
            print(f"Status changed from new to active for key {user_key.key}")
            ...
            db.session.commit()
            print("Earnings committed successfully")
        else:
            print(f"Status did not change from new to active: old_status={old_status}, new_status={user_key.status}")
...
def calculate_earnings(user_key, previous_end_date):
    try:
        print(f"calculate_earnings called for key {user_key.key}, previous_end_date={previous_end_date}")
        ...
        print(f"Calculated extension duration: {extension_duration} days")
        ...
        db.session.commit()
        print("Earnings for extension committed successfully")
    except Exception as e:
        print(f"Error occurred: {e}")
...
# Эндпоинт для приема платежа с проверкой secret_key
...
def process_payment():
    try:
        ...
        # Извлечение данных из JSON с преобразованием типов
        user_id = int(data.get('user_id'))  # Преобразуем в int
        tariff_id = int(data.get('tariff_id'))  # Преобразуем в int
        amount = float(data.get('amount'))  # Преобразуем в float
        payment_date = datetime.strptime(data.get('payment_date'), "%Y-%m-%d %H:%M:%S")  # Преобразуем в datetime
        extension_days = int(data.get('extension_days'))  # Преобразуем в int
        payment_id = data.get('payment_id')
        key = data.get('key')

        # Проверка обязательных полей
        if not all([user_id, tariff_id, amount, extension_days, payment_id, key]):
        ...
        new_subscription_end_date = payment_date + timedelta(days=extension_days)

        # Возвращаем статус и новую дату окончания подписки
        return jsonify({
            'status': True,
            'new_subscription_end_date': new_subscription_end_date.strftime("%Y-%m-%d %H:%M:%S")
        }), 200
